...
/*	if !*toFiles {
	...
	os.Stdout.WriteString("DELETE FROM root_trust_purpose WHERE TRUST_CONTEXT_ID = 5 AND TRUST_PURPOSE_ID < 100;\n")
...
func outputTrustedCerts(out *os.File, objects []*Object) {
	...
	for _, cert := range certs {
		...
		emailDistrustAfter := "NULL"
		if parsed_date, err := time.Parse("060102150405Z", string(cert.attrs["CKA_NSS_SERVER_DISTRUST_AFTER"].value)); err == nil {
			serverDistrustAfter = "'" + parsed_date.Format(time.RFC3339) + "'"
		}
		if parsed_date, err := time.Parse("060102150405Z", string(cert.attrs["CKA_NSS_EMAIL_DISTRUST_AFTER"].value)); err == nil {
			emailDistrustAfter = "'" + parsed_date.Format(time.RFC3339) + "'"
		}
		...
		serverTrusted := false
		if fmt.Sprintf("%s", trust.attrs["CKA_TRUST_SERVER_AUTH"].value) == "CKT_NSS_TRUSTED_DELEGATOR" {
			serverTrusted = true
		}
		emailTrusted := false
		if fmt.Sprintf("%s", trust.attrs["CKA_TRUST_EMAIL_PROTECTION"].value) == "CKT_NSS_TRUSTED_DELEGATOR" {
			emailTrusted = true
		}

		trusted := (serverTrusted || emailTrusted)
		...
		out.WriteString("-- " + nameToString(x509.Subject) + "\n")
		out.WriteString("SELECT import_cert(E'\\\\x" + strings.ToUpper(hex.EncodeToString(derBytes)) + "');\n")
		if serverTrusted {
			out.WriteString("INSERT INTO root_trust_purpose (CERTIFICATE_ID, TRUST_CONTEXT_ID, TRUST_PURPOSE_ID, NOTBEFORE_UNTIL) SELECT c.ID, 5, 1, " + serverDistrustAfter + " FROM certificate c WHERE digest(c.CERTIFICATE,'sha1')=E'\\\\x" + fingerprintString(crypto.SHA1, x509.Raw) + "';\n")
		}
		if emailTrusted {
			out.WriteString("INSERT INTO root_trust_purpose (CERTIFICATE_ID, TRUST_CONTEXT_ID, TRUST_PURPOSE_ID, NOTBEFORE_UNTIL) SELECT c.ID, 5, 3, " + emailDistrustAfter + " FROM certificate c WHERE digest(c.CERTIFICATE,'sha1')=E'\\\\x" + fingerprintString(crypto.SHA1, x509.Raw) + "';\n")
		}
	}
}
